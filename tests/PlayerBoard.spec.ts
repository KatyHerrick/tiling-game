...
describe('PlayerBoard', function() {
  it('has all of the proper components', function() {
  ...
  it('initializes with 0 points', function() {
    const player = new PlayerBoard();
    expect(player.points).toBe(0);
  });

  it('can take tiles given a display and color', function() {
    const player = new PlayerBoard();
  ...
  it('can move tiles from a display to the staging area', function() {
    const player = new PlayerBoard();
    const display = new FactoryDisplay();
    const tilesToDeal = [
      new Tile('red'),
